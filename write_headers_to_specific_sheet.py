...
try:
    target_gid = 394669539
    sheet = spreadsheet.get_worksheet_by_id(target_gid)
    print(f"Found sheet by GID: {sheet.title}")
except Exception as e:
    # 만약 에러가 나면 타이틀로 찾기 시도
    sheets = spreadsheet.worksheets()
    for s in sheets:
        print(f"Sheet available: Title='{s.title}', GID={s.id}")
    sheet = spreadsheet.worksheet("수탁 요청")
...
print(f"Headers inserted successfully into '{sheet.title}'!")
